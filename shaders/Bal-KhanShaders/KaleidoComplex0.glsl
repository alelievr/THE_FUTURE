...
/*        z.xy = .1*(z.xy-cdiv(g(z.xy+of-2.*vec2(cos(1.*t/5.0+float(ii)*param_i ),sin(1.*t/3.0+float(ii)*param_i ))),
        ...
        z.xy = float(ii)*.1251*0.+cmult(z.xy, (vec2(sin(t*.3+.0*float(ii)), cos(t*.3+.0*float(ii))))).xy;
        z.xy = -abs(z.xy)+10.5;//-.01*float(ii);
	     z.xy = cmult(z.xy, vec2(1., -1.) );	
        ...
        if (dist.x > 1000.0)// || dist.y > 100000000000.0)
            break;
    }
    //col /= vec3(float(ii));
