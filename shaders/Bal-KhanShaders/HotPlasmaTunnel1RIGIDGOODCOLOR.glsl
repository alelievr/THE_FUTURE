...
//    rotate(pr.zy, cos(a) );
    ...
    //mind = length(pr.yyxx)-.35; // this is the grid
    id_t = mind;
    mind = min(mind, (/*.25*length(pr.xzy)/ */length(pr.xyz)-.3025 ) ); // this is the blobs/stripes/thingys
    id_t = (id_t != mind)? 1. : 0. ; // used for coloring
    return (mind);
}

float	de_1(vec3 p) // cylinder
{//return (length(fract(p.zz))*-1.125+length(fract(p.x))-8.5);
	  float	mind = 1e5;
	  vec3	pr = p;

    pr.x/=max(.01,sin( (pr.y)*.3675-.5+(sin(pr.z*.25))*.3)-.5)*9.;//pr*=6.;//return(length(pr.xx*pr.zz*pr.yy)-10.);	
    //pr.x/=max(.05,fract( length(sin(p.z*.25) + p.y*.125))-.5)/*+max(.0,fract(p.y*.25)+.0)*/;//*9.;
    //pr.x/= (fract( abs(pr.y)*.06125-.9+(sin(pr.z*.5))*.1)-.5);//*9.;
    vec2     q;
    
    q = vec2(length(pr.yx) - 3., pr.z );
...
float	de_2(vec3 p)
{
    return /*20.-de_1(p);*/(de_0(p)-de_1(p)*.125);
}
...
float	scene(vec3 p)
{
    ...
    #else
    a = ( .8*(p.y*.015 + p.x*.015 + p.z *.15)  + t*3.);
    vec2  rot = vec2( cos(1.57), sin(1.57) );
    #endif
