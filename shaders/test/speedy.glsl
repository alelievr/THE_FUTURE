...
float	scene( vec3 p ) {
    float	mind = 1e5;
    mind = length(p) - .25;// * (phase-.125);
    //mind = sdTriPrism( -p.xzy, vec2(.2, .8) );
    //mind = min(length(simplex3d_fractal(vec3(mind, .2 * iGlobalTime, p.y)) - .1), mind);
    //mind = min(length(simplex3d_fractal(vec3(mind, mind, p.y)) - .1), mind);
 ...
 /*   
   ...
   */ 
    //mind = length(texture(iChannel1,p.xy).x) -.5;
//
   // mind = min(length(noise(vec2(mind, p.y))) -.1, mind);
//
    float fun = max(1.-1./mind, texture(iChannel0, p.xy).x);
//    float fun = min(mind, length(texture(iChannel1,p.xy).x) -.5);
//    mind = min(length(noise(vec2(mind, .33 + cos(phase*5.) *.05))) -.1, mind);
//    mind = min(length(noise(vec2(p.y-1., mind))) -.1, mind);

//    mind = min(mind, fun);
//    mind = min(length(texture(iChannel0, uv)) -length(p.x), mind);
//    mind = length(noise(vec2(iGlobalTime*.25, 2.*p.y)) -.5*mind);
//    mind = mod(mind, 2.);
/*
	vec4 n = vec4(mind, 1., phase-1.5, 0.);
	return( max(mind, dot( p, n.xyz ) + n.w) );
*/
//    return (mind);
/*
    mind = min(length(noise(vec2(mind, .2 * iGlobalTime))) -.1, mind);
    mind = min(length(noise(vec2(p.y*2., mind))) -.1, mind);
    return( max(mind, length(p) - 1.2) );
*/
//
    vec4 n = vec4(0., .0, 1., .7);
    float fin = dot( p, n.xyz ) + n.w;
    mind = min(length(noise(vec2(fin, .3 + /*cos(phase*5.)*/ .01))) -.1, mind);
    mind = min(length(noise(vec2(p.y-1., mind))) -.1, mind);
    return( min(max(mind, length(p) - .2555 -  phase *.3), fun) );
//
}
